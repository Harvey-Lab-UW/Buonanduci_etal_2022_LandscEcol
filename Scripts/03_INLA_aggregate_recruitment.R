...
# Aggregate overstory recruitment INLA analysis
...
dat_pre <- drop_na(dat, live_BA_LS_89_prop) 
dat_post <- drop_na(dat, live_BA_LS_04_prop)
...
# Pre-outbreak ingrowth
stk_pre_I <- inla.stack(
  data = list(y = dat_pre_std$ingrow_stems_89_04),
  A = list(A_pre, 1), 
  effects = list(list(s = 1:spde$n.spde), #spatial random effects
                 data.frame(Intercept = 1, 
                            BA_start = dat_pre_std$live_BA_89,
                            BA_mort_prop = dat_pre_std$mort_BA_89_04_prop,
                            BA_LS_prop_start = dat_pre_std$live_BA_LS_89_prop,
                            Stems_start = dat_pre_std$live_stems_89,
...
# Post-outbreak ingrowth
stk_post_I <- inla.stack(
  data = list(y = dat_post_std$ingrow_stems_04_18),
  A = list(A_post, 1), 
  effects = list(list(s = 1:spde$n.spde), #spatial random effects
                 data.frame(Intercept = 1, 
                            BA_start = dat_post_std$live_BA_04,
                            BA_mort_prop = dat_post_std$mort_BA_04_18_prop,
                            BA_LS_prop_start = dat_post_std$live_BA_LS_04_prop,
                            Stems_start = dat_post_std$live_stems_04,
...
# Ingrowth release
stk_dif_I <- inla.stack(
  data = list(y = dat_post_std$ingrow_stems_dif),
  A = list(A_post, 1), 
  effects = list(list(s = 1:spde$n.spde), #spatial random effects
                 data.frame(Intercept = 1, 
                            BA_start = dat_post_std$live_BA_04,
...
form <- y ~ BA_mort_prop + BA_start + Stems_start + BA_LS_prop_start + 
  Slope + TPI + f(Plot, model="iid") + f(s, model=spde)
...
## Null model ---------
res_dif_I_null <- inla(form_null, 
                       family = "gaussian",
                       data = inla.stack.data(stk_dif_I), 
                       quantiles=c(0.025, 0.5, 0.975),
                       control.compute = list(cpo = TRUE, waic=TRUE),
                       control.predictor = list(A = inla.stack.A(stk_dif_I), 
                                                compute = TRUE))

## Pre-outbreak model ---------
res_pre_I <- inla(form, 
                  family = "poisson",
                  data = inla.stack.data(stk_pre_I), 
                  quantiles=c(0.025, 0.5, 0.975),
                  control.compute = list(cpo = TRUE, waic=TRUE),
                  control.predictor = list(A = inla.stack.A(stk_pre_I), 
                                           compute = TRUE))

## Post-outbreak model ---------
res_post_I <- inla(form, 
                   family = "poisson",
                   data = inla.stack.data(stk_post_I), 
                   quantiles=c(0.025, 0.5, 0.975),
                   control.compute = list(cpo = TRUE, waic=TRUE),
                   control.predictor = list(A = inla.stack.A(stk_post_I), 
                                            compute = TRUE))
